...
class SubscriptionViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.subscription = Subscription.objects.create(
            app=self.app, plan=self.free_plan, is_active=True
        )
    ...
    def test_update_subscription_by_app(self):
        ...
        response = self.client.put(
            reverse("subscription", args=[self.app.id]), data=data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_subscription_not_found(self):
        ...
        data = {"plan": self.standard_plan.id}
        response = self.client.put(
            reverse("subscription", args=[99999]), data=data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
